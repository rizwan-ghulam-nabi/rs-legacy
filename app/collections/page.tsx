...
const quickSearchSuggestions = [
  {
    ...
    queries: ["new products", "latest items", "just arrived", "recent additions"],
    filter: "new",
    icon: Star,
  ...
  },
  {
    ...
    queries: ["popular items", "top products", "best selling", "trending"],
    filter: "bestsellers",
    icon: Crown,
  ...
  },
  {
    ...
    queries: ["limited stock", "exclusive items", "special edition", "rare finds"],
    filter: "limited",
    icon: Gem,
  ...
  {
    category: "Sale Items",
    queries: ["discounted", "on sale", "clearance", "special offers"],
    filter: "sale",
    icon: '⚡',
...
export default function Header() {
  ...
  useEffect(() => {
    if (searchQuery.trim().length > 0) {
      ...
      for (const category of quickSearchSuggestions) {
      ...
      // Add generic product search suggestion
      if (suggestions.length === 0) {
        suggestions.push({
          type: 'generic',
          text: `Search for "${query}" in all products`,
          query: query,
          icon: Search,
          gradient: 'from-purple-500 to-blue-500'
        });
      ...
      setSearchSuggestions(suggestions.slice(0, 4)); // Limit to 4 suggestions
  ...
  // Perform search with enhanced logic
  const performSearch = (query: string) => {
    ...
    for (const category of quickSearchSuggestions) {
      for (const keyword of category.queries) {
        if (lowerQuery.includes(keyword) || keyword.includes(lowerQuery)) {
          ...
          router.push(`/Product?filter=${category.filter}&search=${encodeURIComponent(query)}`);
          setIsSearchOpen(false);
          setSearchQuery('');
          setShowSuggestions(false);
          closeAllMenus();
    ...
    // If no category match, redirect to products with search query
  ...
  // Handle quick search suggestion click
  const handleSuggestionClick = (suggestion: any) => {
    if (suggestion.type === 'category') {
      router.push(`/Product?filter=${suggestion.filter}&search=${encodeURIComponent(searchQuery)}`);
    } else {
      router.push(`/Product?search=${encodeURIComponent(suggestion.query)}`);
    }
    setIsSearchOpen(false);
    setSearchQuery('');
    setShowSuggestions(false);
      ...
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-1000 ${
        ...
        <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 relative" ref={menuRef}>
          <div className="flex justify-between items-center h-20">
            ...
            {/* Enhanced Digital Clock with Date - Hide when search is open */}
            {currentTime && !isSearchOpen && (
                    ...
                    {isDropdownOpen && (
                      <div className="absolute top-full left-0 mt-2 w-80 bg-white/95 dark:bg-gray-900/95 backdrop-blur-3xl rounded-3xl shadow-2xl shadow-purple-500/20 border border-white/20 dark:border-gray-700/30 py-4 z-60 overflow-hidden">
                        ...
                        <div className="relative z-10 px-2">
                          {getFilteredDropdownLinks().map((link, index) => {
            ...
            {/* Enhanced Action Buttons - Modified to handle search state */}
          ...
          {/* Enhanced Search Interface - Full width when active */}
          {isSearchOpen && (
                      ...
                      {searchSuggestions.map((suggestion, index) => {
                        ...
                        return (
                          <button
                            ...
                            className="w-full flex items-center justify-between px-4 py-3 text-gray-700 dark:text-gray-300 hover:text-purple-600 dark:hover:text-purple-400 transition-all duration-300 group hover:bg-purple-50/50 dark:hover:bg-purple-900/20 rounded-2xl mx-2"
                          >
                            <div className="flex items-center space-x-3">
                              {typeof Icon === 'string' ? (
                                <span className="text-lg">{Icon}</span>
                              ...
                              <div className="flex flex-col items-start">
                                <span className="font-medium text-sm">
                                  {suggestion.type === 'category' ? suggestion.category : suggestion.text}
                                </span>
                                {suggestion.type === 'category' && (
                                  <span className="text-xs text-gray-500 dark:text-gray-400">
                                    {suggestion.queries.slice(0, 2).join(', ')}
                                  </span>
                                )}
                              </div>
                            </div>
                            <span className={`px-2 py-1 text-xs font-bold rounded-full bg-gradient-to-r ${suggestion.gradient} text-white transform group-hover:scale-110 transition-transform duration-300`}>
                              {suggestion.type === 'category' ? 'Category' : 'Search'}
                            </span>
                          </button>
                        );
                      })}
                      ...
                      {searchSuggestions.map((suggestion, index) => {
                        ...
                        return (
                          <button
                            key={index}
                            onClick={() => handleSuggestionClick(suggestion)}
                            className="w-full flex items-center justify-between py-3 px-4 text-gray-700 dark:text-gray-300 hover:text-white font-semibold rounded-2xl transition-all duration-500 hover:bg-gradient-to-r hover:from-purple-500/25 hover:to-blue-500/25 group border border-white/20 dark:border-gray-700/30"
                          >
                            <div className="flex items-center space-x-3">
                              {typeof Icon === 'string' ? (
                                <span className="text-lg">{Icon}</span>
