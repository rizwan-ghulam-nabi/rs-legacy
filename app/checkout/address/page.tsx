...
export default function CheckoutAddressPage() {
  const { state: cartState, updateShippingAddress, clearCart } = useCart();
  ...
  // Send FIRST EMAIL when address is selected
  const sendAddressEmailNotification = async (addressData: Address): Promise<string> => {
    try {
      setEmailStatus({ type: 'info', message: 'Sending order notification...' });
      ...
      const orderEmailData = {
        ...
        customerAddress: `${addressData.addressLine1}, ${addressData.city}, ${addressData.state} ${addressData.zipCode}, ${addressData.country}`,
      ...
      if (emailSent) {
        setEmailStatus({ 
          type: 'success', 
          message: `📧 Order #${orderNumber} notification sent to admin!`,
          orderNumber 
        });
      } else {
        setEmailStatus({ 
          type: 'error', 
          message: '⚠️ Order saved but email failed. Proceeding anyway...',
          orderNumber 
        });
      }
      ...
      setEmailStatus({ 
        type: 'error', 
        message: '❌ Email failed, but order will be saved.',
      });
  ...
  const handleContinueToPayment = async () => {
    ...
    try {
      ...
      if (selectedAddressData) {
        ...
        const orderNumber = await sendAddressEmailNotification(selectedAddressData);
        ...
        // Save complete order data for payment page
        const checkoutData = {
          address: selectedAddressData,
          cart: cartState.items,
          totals: { subtotal, shippingCost, tax, finalTotal },
          orderNumber,
  ...
  const EmailStatusDisplay = () => {
    if (!emailStatus.type) return null;
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            {emailStatus.orderNumber && (
              <p className="text-sm text-gray-600 mt-1">
                Order Number: <span className="font-mono font-bold">{emailStatus.orderNumber}</span>
              </p>
            )}
          </div>
        </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="relative">
                  ...
                  <div className="absolute -bottom-6 left-1/2 transform -translate-x-1/2 whitespace-nowrap text-xs font-medium text-purple-600">
                    Address & Email
                  </div>
                </div>
                <span className="font-semibold text-gray-900">Address</span>
                ...
                >
                  ...
                  <div className="mb-6 p-4 bg-blue-50 rounded-xl border border-blue-100">
                    ...
                    <ul className="space-y-2 text-sm text-blue-800">
                      <li className="flex items-center gap-2">
                        ...
                        <span>Email #1: Order placed notification (Now)</span>
                      </li>
                      <li className="flex items-center gap-2">
                        <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                        <span>Email #2: Payment confirmation (Next page)</span>
                      </li>
                    </ul>
                  </div>
                  ...
                  >
                    {isLoading ? (
                      <>
                        <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
                        Sending Email & Processing...
                      </>
                    ) : (
                      <>
                        <Send className="w-5 h-5" />
                        Send Notification & Continue to Payment
                      </>
                    )}
                  </motion.button>
